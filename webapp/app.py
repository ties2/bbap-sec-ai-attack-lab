...
# CORS(app)
#
# # Register the Prompt Injection simulation routes
...
# Path to your new React Vite build
...
@app.route("/")
def index():
    """Serves the new React Dashboard."""
    logger.debug("Serving React dashboard index page")
    return send_from_directory(REACT_BUILD_DIR, "index.html")

@app.route("/old-dashboard")
def old_index():
    """Serves the original Flask template dashboard."""
    logger.debug("Serving old dashboard index page")
    return render_template("index.html")

@app.route("/assets/<path:path>")
def serve_vite_assets(path):
    """Serves the compiled JS and CSS files generated by Vite."""
    return send_from_directory(REACT_BUILD_DIR / "assets", path)

@app.route("/<path:path>")
def catch_all(path):
    """
    Catch-all route to serve static files (like Vite.svg) or fallback
    to index.html for React Router to handle client-side routing.
    """
    # Do not intercept API or static requests
    if path.startswith("api/") or path.startswith("static/"):
        return jsonify({"error": "Not found"}), 404

    # If the file exists in the build dir, serve it directly
    build_file = REACT_BUILD_DIR / path
