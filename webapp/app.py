...
from webapp.routes_sandbox import get_manager, sandbox_bp
from webapp.security import audit_log, configure_security, setup_rate_limiting
...
# ══════════════════════════════════
#  GLOBAL API AUTH GUARD (Critical hardening)
# ══════════════════════════════════


@app.before_request
def require_auth_for_api_routes():
    """Require Bearer token for all /api/* routes except login and preflight."""
...
def serve_react():
    ...
    if REACT_BUILD_DIR.exists():
        ...
        logger.warning(
            f"React build not found at {REACT_BUILD_DIR}, falling back to legacy template"
        )
...
def catch_all(path):
    ...
    if (
        path.startswith("api/")
        or path.startswith("prompt-injection")
        or path == "old-dashboard"
    ):
...
def run_attack(attack_id):
    ...
    commands = {
        "adversarial": [
            "python",
            "-m",
            "src.attacks.adversarial",
            "--attack",
            params.get("attack_type", "both"),
            "--epsilon",
            str(params.get("epsilon", 0.03)),
            "--dataset",
            params.get("dataset", "mnist"),
            "--sweep",
            "--output",
            output_file,
        ],
        "data_poisoning": [
            "python",
            "-m",
            "src.attacks.data_poisoning",
            "--strategy",
            params.get("strategy", "label_flip"),
            "--poison-rate",
            str(params.get("poison_rate", 0.1)),
            "--dataset",
            params.get("dataset", "mnist"),
            "--output",
            output_file,
        ...
        "model_extraction": [
            "python",
            "-m",
            "src.attacks.model_extraction",
            "--strategy",
            params.get("strategy", "random"),
            "--queries",
            str(params.get("num_queries", 1000)),
            "--dataset",
            params.get("dataset", "mnist"),
            "--output",
            output_file,
...
if __name__ == "__main__":
    ...
    logger.info(
        f"React build: {REACT_BUILD_DIR} ({'found' if REACT_BUILD_DIR.exists() else 'NOT FOUND'})"
    )
